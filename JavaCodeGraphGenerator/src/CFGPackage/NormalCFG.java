...
public class NormalCFG implements CFGInterface{
			...
			if(isMethodCall(javaContentData[i]) == true){
				...
				for(int x = 0; x < allIfStatements.size();x++){
					...
					if(currentIfStatement[0].contains(javaContentData[i].trim()) ){
						...
						//THIS SINGLE LINE DOES THE SAME THAT THE NEXT 20 LINES OF CODE
						previousNodeIdentifier = preJoinNodeProcess(newNode, currentNodes, newEdge, currentEdges, 
								nodeNumbering, linesOfCode, javaContentData[i].trim(), 
								previousNodeIdentifier, identifier);
						/*
						...
						//THIS SINGLE LINE DOES THE SAME THAT THE NEXT 20 LINES OF CODE
						identifier = postJoinNodeProcess(previousNodeIdentifier);
	...
	private int postJoinNodeProcess(int previousNodeIdentifier){
